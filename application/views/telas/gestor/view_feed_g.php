...
<div class="content-wrapper">
	...
	<section class="content">
		<?php $this->load->view ( 'telas/gestor/view_flashdata_g' ); ?>
		<?php foreach ( $dados
		...
		as $q ){
		$like = $this->model_gestor->l_contagem ( $q->publicacao_id );
		$coment = $this->model_gestor->c_contagem ( $q->publicacao_id );
		$destaque = $this->model_gestor->c_destaque ( $q->publicacao_id );

		$postador = $this->model_gestor->busca ( $q->user_id );
		foreach ( $postador
			...
			<div class="col-md-12">
				...
				<div class="box box-widget">
					...
					<div class="box-body">
							...
							<tr>
								...
								<td>
									<form
										...
										<button type="submit"
											<?php
											$verifica = $this->model_gestor->ver_like ( $id , $q->publicacao_id );
											?>
						...
						<a href="<?php echo base_url ( 'g/p/' . $q->publicacao_id ); ?>"><span
								class="pull-right text-muted"><?php echo $like . ' ' . $qtd_likes . ' | ' . $coment . ' Comentários'; ?></span></a>
					</div>
					<!-- /.box-body -->
					<div class="box-footer box-comments">
						<a href="<?php echo base_url ( 'g/p/' . $q->publicacao_id ); ?>">
							<?php
							foreach ($destaque as $des) {
								$dest = $this->model_gestor->busca ( $des->user_id );
								?>
...
<script>
    ...
    $(document).ready(function () {
        $('.bloqPubli').click(function () {
            ...
            $('#frmPublicacaoB').attr('action', $('#base_url').val() + 'g/bloquear/' + id_publicacao);
        });
    });
    $(document).ready(function () {
        $('.excluirPubli').click(function () {
            ...
            $('#frmPublicacaoD').attr('action', $('#base_url').val() + 'g/excluir/' + id_publicacao);
        });
    });
    $(document).ready(function () {
        $('.ocultarPubli').click(function () {
            ...
            $('#frmPublicacaoO').attr('action', $('#base_url').val() + 'g/ocultar/' + id_publicacao);
        });
    });
