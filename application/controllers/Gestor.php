...
class Gestor extends CI_Controller
{
	...
	function __construct ()
	{
		...
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $dado[ 'profile_id' ] != 3 ) {

				$dados = base64_encode ('Gestor');

				redirect('autorizacao/verificar/'.$dados);
			}
		}
	...
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			redirect ( 'gestor/inicio' , 'refresh' );
		}
		else {
	...
	function inicio ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			$session = $this->session->userdata ( 'logged_in' );
			$this->load->model ( "model_gestor" );
			$id = $session[ "user_id" ];
			$resultado = $this->model_gestor->busca ( $id );

			$dados[ 'requisicao' ] = $resultado;

			$total_posts = $this->model_gestor->total_posts ();

			$this->load->library ( 'pagination' );

			$maximo = 15;
			$inicio = ( !$this->uri->segment ( "3" ) ) ? 0 : $this->uri->segment ( "3" );
			$config[ 'base_url' ] = base_url ( 'gestor/feed' );
			...
			$dados[ "emails" ] = $this->model_gestor->trazer_posts ( $maximo , $inicio );


			$dados[ 'tela' ] = 'view_feed_g';

			$this->load->view ( 'view_home_gestor' , $dados );
		}
		else {
			redirect ( 'login' , 'refresh' );
			session_destroy ();
	...
	function mensagens ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$total_emails = $this->model_gestor->recebidos_l ( $id );

			$this->load->library ( 'pagination' );

			$maximo = 10;
			$inicio = ( !$this->uri->segment ( "3" ) ) ? 0 : $this->uri->segment ( "3" );
			$config[ 'base_url' ] = base_url ( 'gestor/mensagens' );
			...
			$dados[ 'paginacao' ] = $this->pagination->create_links ();
			$dados[ "emails" ] = $this->model_gestor->recebidos_d ( $id , $maximo , $inicio );

			$resultado = $this->model_gestor->busca ( $id );

			$dados[ 'requisicao' ] = $resultado;

			$dados[ 'tela' ] = 'view_mensagens_g';
	...
	function curso ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$total_posts = $this->model_gestor->total_p_c ( $curso[ 'user_curso' ] );

			$this->load->library ( 'pagination' );

			$maximo = 15;
			$inicio = ( !$this->uri->segment ( "3" ) ) ? 0 : $this->uri->segment ( "3" );
			$config[ 'base_url' ] = base_url ( 'gestor/curso' );
			$config[ 'total_rows' ] = $total_posts;
			$config[ 'per_page' ] = $maximo;
			$config[ 'display_pages' ] = FALSE;
			...
			$dados[ 'paginacao' ] = $this->pagination->create_links ();
			$dados[ "emails" ] = $this->model_gestor->trazer_p_c ( $maximo , $inicio , $curso[ 'user_curso' ] );

			###############################

			$dados[ 'tela' ] = 'view_curso_g';
	...
	function enviar_arquivo ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$dados[ 'tela' ] = 'view_enviar_conteudo_g';
	...
	function enviar_texto ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$dados[ 'tela' ] = 'view_enviar_texto_g';
	...
	function calendario ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$calendar = $this->model_gestor->calendar ();

			$dados[ 'calendar' ] = $calendar;
			$dados[ 'eventos' ] = $this->model_gestor->meus_eventos ( $id );

			$dados[ 'tela' ] = 'view_calendario_g';
	...
	function apagar_evento ( $e_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			$this->load->model ( 'model_gestor' );
			$this->model_gestor->apagar_evento ( $e_id );
	...
	function contatar ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $this->form_validation->run () == FALSE ) {
				...
				$this->load->model ( 'model_gestor' );
				$return = $this->model_gestor->contatar ( $user_data );
	...
	function ler_recebido ( $mail_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			if ( $mail_id == null ) {
				...
				$this->load->model ( 'model_gestor' );
				$email = $this->model_gestor->mail_recebido ( $mail_id );
				$dados[ 'email' ] = $email;
				$dados[ 'tela' ] = 'view_ler_recebido_g';
	...
	function ler_enviado ( $mail_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			if ( $mail_id == null ) {
				...
				$this->load->model ( 'model_gestor' );
				$email = $this->model_gestor->mail_enviado ( $mail_id );
				$dados[ 'email' ] = $email;
				$dados[ 'tela' ] = 'view_ler_enviado_g';
	...
	function envio ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $this->form_validation->run () == FALSE ) {
				...
				$result = $this->model_gestor->verifica ( $emailto );
				if ( isset( $result ) ) {
					$data = date ( 'd-m-Y' );
					$hora = date ( 'H:i' );
					$id = $session[ 'user_id' ];
					$dados = $this->model_gestor->busca ( $id );
	...
	function perfil ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			$session = $this->session->userdata ( 'logged_in' );
			$dados[ 'tela' ] = 'view_perfil_g';
			$this->load->model ( 'model_gestor' );
			$id = $session[ 'user_id' ];
			$result = $this->model_gestor->busca ( $id );
			$dados[ 'user' ] = $result;
			$this->load->view ( 'view_home_gestor' , $dados );
		}
		else {
	...
	function foto ()
	{
		...
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $this->upload->do_upload ( 'foto' ) ) {
				...
				$this->load->model ( 'model_gestor' );
				$this->model_gestor->perfil ( $id , $img );
	...
	function apagar_enviado ( $email_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			if ( $email_id != null ) {
				...
				$this->model_gestor->apagar_enviada ( $email );
	...
	function apagar_recebido ( $email_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			if ( $email_id != null ) {
				...
				$this->session->set_flashdata ( 'sucesso' , 'Apagado com sucesso!' );
				$this->model_gestor->apagar_recebida ( $email );
	...
	function enviadas ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$total_emails = $this->model_gestor->enviados_l ( $id );

			$this->load->library ( 'pagination' );

			$maximo = 10;
			$inicio = ( !$this->uri->segment ( "3" ) ) ? 0 : $this->uri->segment ( "3" );
			$config[ 'base_url' ] = base_url ( 'gestor/enviadas' );
			$config[ 'total_rows' ] = $total_emails;
			$config[ 'per_page' ] = $maximo;
			$config[ 'display_pages' ] = FALSE;
			...
			$dados[ 'paginacao' ] = $this->pagination->create_links ();
			$dados[ "emails" ] = $this->model_gestor->enviados_d ( $id , $maximo , $inicio );

			$resultado = $this->model_gestor->busca ( $id );

			$dados[ 'requisicao' ] = $resultado;

			$dados[ 'tela' ] = 'view_enviadas_g';
	...
	function responder ( $email_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			if ( $email_id != null ) {
				$dados[ 'tela' ] = 'view_responder_g';

				$this->load->model ( 'model_gestor' );
				$dados[ 'emails' ] = $this->model_gestor->mail_responder ( $email_id );
	...
	function resposta ()
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $this->form_validation->run () == FALSE ) {
				...
				redirect ( 'gestor/mensagens' , 'refresh' );
			}
			else {
				$session = $this->session->userdata ( 'logged_in' );
				$emailto = $this->input->post ( 'emailto' );
	...
	function curtir ( $p_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$verifica = $this->model_gestor->ver_like ( $id , $p_id );

			if ( empty( $verifica ) ) {

				$this->model_gestor->curtir ( $id , $p_id );
				...
				$this->model_gestor->descurtir ( $id , $p_id );
	...
	function comentar ( $p_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			if ( $this->form_validation->run () == FALSE ) {
				...
				$this->model_gestor->comentar ( $id , $p_id , $comentario );
	...
	function publicacao ( $p_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			//die();
			$this->load->view ( 'template/gestor/header' );
			$this->load->view ( 'template/gestor/topbar' );
			$this->load->view ( 'template/gestor/sidebar' );
			$this->load->view ( 'template/gestor/configbar' );

			$this->load->view ( 'telas/gestor/view_publicacao_p' , $dados );

			$this->load->view ( 'template/gestor/footer' );
			$this->load->view ( 'template/gestor/controlbar' );
			$this->load->view ( 'template/gestor/js' );
	...
	function responder_c ( $p_id , $c_id )
	{
		if ( $this->session->userdata ( 'logged_in' ) ) {
			...
			$this->load->model ( 'model_gestor' );
			$this->model_gestor->responder_c ( $resposta );
